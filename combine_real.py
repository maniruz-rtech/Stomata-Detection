...
while True:
    ...
    gray_filtered = cv2.inRange(blur, 154, 255)
    ...
    gray_filtered2 = cv2.inRange(blur, 176, 255,cv2.THRESH_TOZERO)
    kel1 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5))
    erosion1 = cv2.erode(gray_filtered2, kel1, iterations=1)
...
cv2.imwrite(VIDEO.replace('.avi', 'combine_rea.png'), ((OUT*255/iter)).astype("uint8"))
cv2.imwrite(VIDEO.replace('.avi', 'combine_real.png'), ((cell*255/iter)).astype("uint8"))
