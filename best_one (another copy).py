...
while True:
    ...
    #DOESN'T WORK gray_filtered1= cv2.adaptiveThreshold(res1,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,11,2)
    OUT += dil_erode
...
cv2.imwrite(VIDEO.replace('.avi', 'best_one(another copy).png'), (OUT/iter).astype("float"))
